...
import { CHILD_OBJECT_INVALID, CHILD_REQUIRED, CHILD_TYPE_INVALID, CHILD_UNKNOWN, FIRST_CHILD_OBJECT_INVALID, FIRST_CHILD_TYPE_INVALID, LAST_CHILD_OBJECT_INVALID, LAST_CHILD_TYPE_INVALID, NODE_DATA_INVALID, NODE_IS_VOID_INVALID, NODE_MARK_INVALID, NODE_TEXT_INVALID, PARENT_INVALID } from 'slate-schema-violations';
...
var Node = function () {
  ...
  createClass(Node, [{
    ...
    value: function getAncestors(key) {
      ...
      if (path) {
        return path.slice(0, -1).reduce(function (ancestors, index) {
    ...
    value: function getCommonAncestor(one, two) {
      ...
      var onePath = this.getPath(one);
      var twoPath = this.getPath(two);
      ...
      var commonAncestorPath = onePath.slice(0, index);
    ...
    value: function getFragmentAtRange(range) {
      ...
      while (node.getParent(child.key)) {
        ...
        node = node.splitNode(node.getPath(child.key), _position);
        previous = node.getNextSibling(child.key);
        child = node.getParent(child.key);
      }
    ...
    value: function getNextSibling(key) {
      ...
      var nextSiblingPath = path.map(function (n, i) {
        return isLast(i) ? n + 1 : n;
      });
    ...
    value: function forEachDescendantWithPath(iterator) {
      var ancestorPath = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [];
      var changedChildIndex = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;
      ...
      this.nodes.forEach(function (child, i) {
        ...
        var path = [].concat(toConsumableArray(ancestorPath), [i]);
    ...
    key: 'regeneratePathsCache',
    value: function regeneratePathsCache() {
      var _this2 = this;

      this.__cache_paths = Map$1();
      this.forEachDescendantWithPath(function (descendant, path) {
        _this2.__cache_paths = _this2.__cache_paths.set(descendant.key, path);
      });
    }
  }, {
    key: 'getPathsCache',
    value: function getPathsCache() {
      return this.__cache_paths;
    }
  }, {
    key: 'setPathsCache',
    value: function setPathsCache(pathsCache) {
    ...
    value: function insertNode(path, node) {
      ...
      if (node.object != 'text') {
        node = node.mapDescendants(function (desc) {
          return _this3.hasDescendant(desc.key) ? desc.regenerateKey() : desc;
        });
      }

      var parent = this.getDescendantAtPath(path.slice(0, -1));
      var index = path[path.length - 1];
      var nodes = parent.nodes.insert(index, node);
    ...
    key: 'mergeNode',
    value: function mergeNode(withPath, path) {
      var one = this.getDescendantAtPath(withPath);
      var two = this.getDescendantAtPath(path);
      ...
      if (one.object == 'text') {
        ...
        var _nodes = one.nodes.concat(two.nodes);
        one = one.set('nodes', _nodes);
      }

      var parent = this.getDescendantAtPath(path.slice(0, -1));
      var nodes = parent.nodes.reduce(function (children, child) {
      ...
      var index = withPath[withPath.length - 1];
      return this.updateNode(parent.set('nodes', nodes), true, index);
    }
    ...
    value: function mapDescendants(iterator) {
      ...
      nodes.forEach(function (node, index) {
        ...
        ret = iterator(ret, index, _this5.nodes);
        if (ret == node) return;
    ...
    value: function removeNode(path) {
      var node = this.getDescendantAtPath(path);
      var parent = this.getParent(node.key);
      var nodes = parent.nodes.filter(function (_ref) {
        var key = _ref.key;
        return key !== node.key;
      });
      var index = path[path.length - 1];
    ...
    key: 'splitNode',
    value: function splitNode(path, position) {
      var descendant = this.getDescendantAtPath(path);
      ...
      if (descendant.object == 'text') {
        var befores = descendant.characters.take(position);
        var afters = descendant.characters.skip(position);
        ...
        var _befores = descendant.nodes.take(position);
        var _afters = descendant.nodes.skip(position);
        one = descendant.set('nodes', _befores);
        two = descendant.set('nodes', _afters).regenerateKey();
      }
    ...
    value: function updateNode(node) {
      var willTreeChanged = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
      var changedChildIndex = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;
      ...
      var pathsCache = this.getPathsCache();
      if (pathsCache && willTreeChanged) {
        var path = this.getPath(child.key);
        if (!Text.isText(child)) {
          child.forEachDescendantWithPath(function (descendant) {
            return pathsCache = pathsCache.remove(descendant.key);
          }, path, changedChildIndex);
        }
        if (!Text.isText(node)) {
          node.forEachDescendantWithPath(function (descendant, descendantPath) {
            return pathsCache = pathsCache.set(descendant.key, descendantPath);
          }, path, changedChildIndex);
        }
      }
      ...
      if (rule.nodes != null || parents != null) {
        ...
        while (nextChild()) {
          if (parents != null && _child2.object != 'text' && _child2.type in parents) {
            ...
            var definedObjects = r.parent.objects != null;
            var definedTypes = r.parent.types != null;

            var allowedObject = definedObjects && r.parent.objects.includes(node.object);
            var allowedType = definedTypes && r.parent.types.includes(node.type);

            var allowed = !definedObjects && !definedTypes || allowedObject || allowedType;
...
var APPLIERS = {
  ...
  move_node: function move_node(value, operation) {
    ...
    document = document.insertNode([].concat(toConsumableArray(newParentPath), [newIndex]), node);
    value = value.set('document', document);
    return value;
  },
  ...
  split_node: function split_node(value, operation) {
    ...
    if (properties) {
      var splitNode = document.getNextSibling(node.key);
